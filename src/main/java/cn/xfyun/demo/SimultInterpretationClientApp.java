...
import cn.xfyun.config.MicrophoneAudioSender;
import cn.xfyun.config.PropertiesConfig;
import cn.xfyun.model.simult.response.Recognition;
import cn.xfyun.model.simult.response.SimultInterpretationResponse;
import cn.xfyun.model.simult.response.Streamtrans;
import cn.xfyun.service.simult.SimultInterpretationWebSocketListener;
 ...
 * 1、APPID、APISecret、APIKey信息获取：https://console.xfyun.cn/services/simult_interpretation
 * 2、文档地址：https://www.xfyun.cn/doc/nlp/simultaneous-interpretation/API.html
 */
public class SimultInterpretationClientApp {
    private static final Logger logger = LoggerFactory.getLogger(SimultInterpretationClientApp.class);
    ...
    static {
        try {
            ...
            resourcePath = SimultInterpretationClientApp.class.getResource("/").toURI().getPath();
    ...
    public static void main(String[] args) throws MalformedURLException, SignatureException, UnsupportedEncodingException, FileNotFoundException {
        SimultInterpretationClient client = new SimultInterpretationClient.Builder()
    ...
    private static void transferToPcmFile(SimultInterpretationClient client) {
        ...
        try {
            ...
            client.send(inputFile, new SimultInterpretationWebSocketListener() {
                @Override
                public void onSuccess(WebSocket webSocket, SimultInterpretationResponse resp) {
    ...
    private static void transferTts(SimultInterpretationClient client) {
        try {
            ...
            client.start(new SimultInterpretationWebSocketListener() {
