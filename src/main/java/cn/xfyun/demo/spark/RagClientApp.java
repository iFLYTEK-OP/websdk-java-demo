...
public class RagClientApp {
    ...
    public static void main(String[] args) {
        try {
            ...
            //         // .notifyUrl("萃取任务回调地址")
            //         .build();
            // String statusResp = client.fileExtract(fileExtract);
            // logger.info("提交萃取任务结果：{}", statusResp);
            String taskId = "";
            ...
            //             .fileId(fileId)
            //             .build();
            //     String extractStatus = client.fileExtractStatus(fileResult);
            //     JSONObject extractStatusJson = JSONUtil.parseObj(extractStatus);
            //     String status = extractStatusJson.getJSONObject("data").getStr("status");
            //     boolean terminal = ExtractionStatus.fromCode(status).isTerminal();
            //     logger.info("文件萃取状态查询结果：{}", extractStatusJson);
            //     if (terminal) {
            //         break;
            //     }
            ...
            //         .taskId(taskId)
            //         .build();
            // String extractResult = client.fileExtractResult(fileResult);
            ...
            //     JSONObject fileSummaryQueryJson = JSONUtil.parseObj(fileSummaryQuery);
            //     String status = fileSummaryQueryJson.getJSONObject("data").getStr("summaryStatus");
            //     SummaryStatus summaryStatus = SummaryStatus.fromCode(status);
            //     logger.info("文件萃取状态查询结果：{}", fileSummaryQueryJson);
            //     if (SummaryStatus.SUMMARYING != summaryStatus) {
            //         break;
            //     }
            //     TimeUnit.MILLISECONDS.sleep(1000);
            // }
            ...
            // 14. 文档向量化
            // String fileEmbeddingResp = client.fileEmbedding(Collections.singletonList(fileId));
